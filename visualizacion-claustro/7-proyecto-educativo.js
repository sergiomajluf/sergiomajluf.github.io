...
function preload() {
    ...
    txt = loadStrings("preguntas/PROYECTO_EDUCATIVO_2020.txt");
...
function instrucciones(ciudad) {
    ...
    text("Frecuencia de conceptos en Proyecto Educativo USS", width / 2, 60);
...
function textos() {
    ...
    for (let i = 0; i < keys.length; i++) {
        ...
        */
        fill(
            100, 120, 175, map(wordCount, min, max, 10, 205)
        )

        // console.log(key, wordCount);
        // rect(acumuladoX, wordY - wordHeight, wordHeight, wordHeight);
        text(key + "(" + wordCount + ")", acumuladoX + random(20), acumuladoY);
...
function histograma(x, y, w, h, keys, counts) {
    ...
    for (let i = 0; i < keys.length; i++) {
        ...
        var lh = map(wordCount, 1, max, 1, h);

        //fill(255, 70);
        fill(20, 70);
        noStroke();
        text(key, -5, i * espacio);
