...
function instrucciones(ciudad) {
    ...
    textFont(f_FiraSans);
    textSize(30);
    ciudad = "Frecuencia de Valores"
    ciudad = capitalize(ciudad);
    text(ciudad, width / 2, 40);
    archivo_salida = ciudad.toLowerCase();
    textSize(11);
    textFont(f_FiraSansLI);
    text("Cómo los valores pueden determinar la práctica de la innovación", width / 2, 60);
...
function textos() {
    ...
    for (let i = 0; i < keys.length; i++) {
        ...
        wordX = textWidth(key) + 25;

        if (acumuladoX > windowWidth - wordX) {
            acumuladoX = 0;
            acumuladoY += wordHeight * 1.5;
        }
